				...
				void (*update)(), my_w_t *window)
{
	...
	window);
	scene_t *scene = hm_get(window->scenes, scene_name);
	...
	if (displayed_scene) {
		...
		window->current = hm_get_bucket(window->scenes, scene_name);
		if (update)
			update(scene, window);
		if (add_scene_to_display_list(hm_get_bucket(window->scenes,
			scene_name), window) != 0)
			return (84);
	}
	return (1);
